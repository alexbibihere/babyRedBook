...
for img in images:
    ...
    if len(parts) >= 4:
        ...
        title_to_images[title].append({
            'path': str(img),
            'filename': img.name
        })
print(f"找到 {len(title_to_images)} 个不同的笔记标题\n")
...
for i, image_path in enumerate(images, 1):
    print(f"[{i}/{total_images}] {image_path.name[:50]}...")
    ...
    try:
        ...
        # 每 10 张显示进度
        if processed % 10 == 0:
            print(f"\n已处理: {processed}/{total_images}\n")

    except Exception as e:
        print(f"  [ERROR] 错误: {e}")

print(f"\n处理完成! 共识别 {processed}/{total_images} 张图片\n")
